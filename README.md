...
這是一個基於 **Spring Boot** 開發的 RESTful API 專案，模擬了現代銀行的核心後端功能。專案的設計目標是展現一個**結構清晰、功能完整、安全可靠**的後端系統。它不僅包含帳戶管理、存提款、轉帳等基本操作，更整合了 **Spring Security 與 JWT** 實現了完整的使用者認證與授權，並在服務層實現了針對**高併發交易的樂觀鎖與悲觀鎖**機制，確保資金數據的一致性。

專案採用分層架構、DTO 模式與全域例外處理等業界標準實踐。為了確保開發與部署環境的一致性，本專案全面支援 **Docker 容器化部署**。透過 **Docker Compose** 進行服務編排，將 Spring Boot 應用程式與 **MySQL 8.0 資料庫** 整合於獨立的容器網路中，實現了包含數據持久化 (Volume) 與自動重啟機制的**一鍵啟動**環境，大幅降低環境建置的門檻。
