...
public class AccountController {
    ...
    @PreAuthorize("hasRole('USER')") // 確保必須有 USER 權限
    @Operation(summary = "取得我的所有帳戶", description = "根據當前登入者的 JWT 取得其名下所有帳戶")
    public ResponseEntity<List<AccountDto>> getMyAccounts() {
        List<AccountDto> myAccounts = accountService.getMyAccounts();
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "帳戶存款", description = "存入資金到指定帳戶 (僅限ADMIN)")
    @ApiResponses(value = {
    ...
    @PreAuthorize("hasRole('ADMIN')")
    @Operation(summary = "帳戶提款", description = "從指定帳戶提取資金 (僅限ADMIN)")
    @ApiResponses(value = {
